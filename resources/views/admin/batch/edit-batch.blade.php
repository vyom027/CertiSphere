    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
        <div class="container-fluid py-1 px-3">
          <nav aria-label="breadcrumb">
            ...
            <h6 class="font-weight-bolder text-white mb-0">Edit Batch</h6>
          </nav>
            ...
            <div class="col-md-9"> <!-- Adjust column width here -->
                <div class="card shadow">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">Edit Batch</h6>
                        <a href="{{ route('batch.index') }}" class="btn btn-light btn-sm">Back to List</a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('batch.update', ['id' => $batch->batch_id]) }}" method="POST">
                          @csrf
                          @method('PUT')
                          ...
                          <div class="mb-4">
                              ...
                              <select name="dept_id" id="department_id" class="form-select" required>
                                  @foreach($departments as $department)
                                      <option value="{{ $department->dept_id }}" {{ $department->dept_id == $batch->dept_id ? 'selected' : '' }}>
                                          {{ $department->name }}
                                      </option>
                                  @endforeach
                              </select>
                          ...
                          <div class="mb-4">
                              ...
                              <input type="number" name="start_year" id="start_year" class="form-control" value="{{ $batch->start_year }}" required>
                          </div>
          
                          <!-- End Year -->
                          <div class="mb-4">
                              <label for="end_year" class="form-label">End Year</label>
                              <input type="number" name="end_year" id="end_year" class="form-control" value="{{ $batch->end_year }}" required>
                          </div>
